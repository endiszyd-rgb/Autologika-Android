...
export function enrichPartFromHtml(item,html=''){
  if(!item)return item
...
export function mapWebSearch(html,scannedCode){
...
async function lookupBarcodeOnlineWithFetch(fetchImpl,value,{details=false}={}){
  ...
  for(const provider of providers){
    try{
      const response=await fetchWithTimeout(fetchImpl,provider.url,{headers:{...headers,...provider.headers}})
...
export {LOOKUP_VERSION}
export async function lookupBarcodeOnline(value,{fetchImpl=fetch}={}){
  const result=await lookupBarcodeOnlineWithFetch(fetchImpl,value,{details:true})
  return result.item?{found:true,barcode:normalizeBarcode(value),item:result.item,source:result.item.lookup_source}:{found:false,barcode:normalizeBarcode(value),unavailable:!result.available,errors:result.errors}
}
